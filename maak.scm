...
(define configs-dir (string-append repo-root "/configs"))
(define configen-dir (string-append configs-dir "/main"))
  ...
  (let ((full-path (canonicalize-path file-path)))
    (when (and (file-exists? full-path)
      ...
      (let ((base-dir (dirname full-path))
            (load-root* (or load-root (dirname full-path))))
        (call-with-input-file full-path
          (lambda (port)
            (let loop ((line (get-line port)))
              (unless (eof-object? line)
                (let ((match (string-match "^[[:space:]]*\\(load[[:space:]]+\"(\\.\\.?/[^\"]+)\"[[:space:]]*\\)[[:space:]]*$" line)))
                  (if match
                      (let* ((relative-path (match:substring match 1))
                             (target-path (if (string-prefix? "./" relative-path)
                                              (string-append load-root* "/" (substring relative-path 2))
                                              (string-append base-dir "/" relative-path))))
                        (display "\n;\n")
                        (format #t "; ====== 来自 ~a ======\n" relative-path)
                        (display ";\n")
...
(define (generate-config input-name output-name desc)
  ...
  (let ((input-path (string-append configen-dir "/" input-name))
    ...
    (set! loaded-files (make-hash-table))
...
(define (style . args)
  "格式化代码"
  (apply $ (list (append (list "guix" "style" "--whole-file") args))))

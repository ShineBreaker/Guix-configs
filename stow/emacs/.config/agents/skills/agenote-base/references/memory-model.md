...
```bash
kb agenote memory                          # 全部概览
kb agenote memory --type feedback          # 只看 feedback
kb agenote memory --project <名称|路径|.>   # 按项目检索
kb agenote memory --get                    # 全文
kb agenote memory --get --type project     # 只看 project 节
kb agenote memory --stale                  # 陈旧记忆（超 30 天未更新）
```
...
echo "用户偏好简洁回复，不要长篇解释" | \
  kb agenote memory --add --type feedback --title "回复风格偏好" --stdin

# project 记忆（按项目拆分到独立文件）
echo "该项目用 Guix 构建，blue rebuild 部署" | \
  kb agenote memory --add --type project --title "构建方式" \
...
```bash
kb agenote memory --touch F001              # 更新时间戳
kb agenote memory --archive F001            # 归档到 deprecated

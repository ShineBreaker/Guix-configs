...
§
用户偏好:使用 `clarify` 工具时,必须把选项列在 `choices` 数组里作为可勾选的 row,**不要**把选项塞进 `question` 文本里。原话:"重新提问一次,你目前并没有成功显示出选项"。前一次失败的提问把所有候选写进 question 字符串里,UI 渲染成不可选的散文。规则:options only in `choices[]`, never inside `question`.
§

--------------

我会尽量地把更多的配置放在这里，而 =system-config.org=
只会放置一些保障系统正常使用所必须的配置，这样的好处在于:

1. 在首次安装系统的时候，可以少安装很多很多的软件包，这样安装系统的时长不会说长的太过分，也可以尽量减少因为网络问题而造成的报错
2. 系统和用户分开配置，这样在修改某一块的配置时不用将一整个系统都 reconfighration 一遍，方便多次迭代

--------------

* Agent 指引

Home 配置修改指引

本区域为 AI 助手提供 Home 配置的上下文和决策规则。阅读此区域后再进行修改。

Home 配置定义 home-environment 的组成部分，包括：
- 用户级包集合：桌面环境、Emacs、终端、开发工具、其他软件包
- Home 服务定义：shepherd 服务、计时器、环境变量、字体配置
- dotfiles 服务配置：通过 home-dotfiles-service-type 分发配置文件

dotfiles 管理规则

dotfiles 目录采用 Stow 规范组织，结构为 dotfiles/<app>/.config/<app>/。
这些配置通过 home-dotfiles-service-type 服务自动链接到用户家目录。
不要随意移动已有目录，保持现有结构稳定。

新增应用配置流程

添加新应用配置时，按以下顺序操作：
1. 在 dotfiles/<app>/ 下创建配置目录，遵循 Stow 规范
2. 在本文件的包列表中添加应用包（如 desktop-packages-list）
3. 在 home-dotfiles-configuration 的 packages 中添加目录名

source/files/ 用途

source/files/ 目录存放静态模板文件，用于需要 computed-substitution-with-inputs 注入 Guix 包绝对路径的场景。
例如 niri.kdl、zed.json 等文件，使用 &&bin/foo&& 语法标记需要替换的二进制路径。

修改约定

- 优先修改 dotfile 文件，若需要 Guix Home 管理再修改本文件
- 保持现有的 (load ...) 风格和变量命名习惯
- 变量名如 %services-config、%packages-list 等保持统一风格

决策规则

- 若应用已有 dotfile 配置，优先修改对应 dotfile
- 新增程序时考虑三处联动：包列表、服务定义、配置文件
- 能用 Home 解决就不用 System，保持系统层最小化

--------------

我会尽量地把更多的配置放在这里，而 =system-config.org=
只会放置一些保障系统正常使用所必须的配置，这样的好处在于:

1. 在首次安装系统的时候，可以少安装很多很多的软件包，这样安装系统的时长不会说长的太过分，也可以尽量减少因为网络问题而造成的报错
2. 系统和用户分开配置，这样在修改某一块的配置时不用将一整个系统都 reconfighration 一遍，方便多次迭代

--------------

* Main

在这里，我利用了 =Org Mode= 的 =Noweb= 功能
这个功能允许我将代码块命名，然后利用命名来进行引用，将所有引用都放到到下面这个代码块中再自动拼合并输出
引用的语法为 *<<>>* , 所以你看到类似的东西时不要困惑，这不是 scheme 原生的功能，是 orgmode 的相关功能

#+NAME: main
#+begin_src scheme :tangle ../../tmp/home-config.scm :noweb yes
(load "../source/information.scm")

<<modules>>

<<custom-packages>>

<<services-modules>>

(home-environment
 (packages (append
            <<desktop-packages-list>>
            <<emacs-packages-list>>
            <<terminal-packages-list>>
            <<devtools-packages-list>>
            <<user-packages-list>>
            ))

 (services (append
            <<user-desktop-services>>
            <<home-shepherd-services>>
            <<timer-services>>
            <<fish-services>>
            <<modprobe-services>>
            <<dotfile-services>>
            <<home-files-services>>
            <<environment-variable-services>>
            <<font-services>>

            %base-home-services)))
#+end_src

* Modules

这些以及下面 =service= 块中的 module 都是为了导入对应变量而使用的，所以必须放置在一个配置文件的最前面

#+NAME: modules
#+begin_src scheme
(use-modules (gnu)
             (gnu home)
             (gnu home services)
             (guix packages)
             (gnu services)

             (jeans packages games)

             (rosenthal utils file)
             (rosenthal utils packages))
#+end_src

* Custom Packages

自定义的 Guix 包定义。

#+NAME: custom-packages
#+begin_src scheme
(use-modules (gnu packages)
             (gnu packages librewolf)
             (gnu system shadow)

             (guix download)
             (guix gexp)
             ((guix licenses) #:prefix license:)
             (guix utils)

             (guix build-system trivial))

(define librewolf-l10n-zh-cn
  (package
    (name "librewolf-l10n-zh-cn")
    (version "149.0")
    (source
     (origin
       (method url-fetch)
       (uri "https://addons.mozilla.org/firefox/downloads/file/4751333/chinese_simplified_zh_cn_la-149.0.20260403.140140.xpi")
       (hash (content-hash (base32 "0mgfy5dpgpf4s16hxkaiv171j3rp13yn6w5bp708lkmbyxydbadp")))))
    (build-system trivial-build-system)
    (arguments
     (list
      #:modules '((guix build utils))
      #:builder
      #~(begin
          (use-modules (guix build utils))
          (let ((ext-dir (string-append #$output
                                        "/lib/librewolf/browser/extensions")))
            (mkdir-p ext-dir)
            (copy-file #$source
                       (string-append ext-dir
                                      "/langpack-zh-CN@firefox.mozilla.org.xpi"))))))
    (home-page "https://addons.mozilla.org/")
    (synopsis "LibreWolf Chinese (Simplified) language pack")
    (description "Firefox zh-CN language pack compatible with LibreWolf 149.0.")
    (license license:mpl2.0)))

;;; LibreWolf + 中文语言包合并包
;;; ---------------------------------------
;;; 复制 LibreWolf 并注入 langpack，同时 patch policies.json 允许加载 langpack。

(define librewolf-with-l10n
  (package
    (inherit librewolf)
    (name "librewolf-with-l10n")
    (source #f)
    (build-system trivial-build-system)
    (arguments
     (list
      #:modules '((guix build utils))
       #:builder
       #~(begin
           (use-modules (guix build utils))

           (define lw #$librewolf)
           (define out #$output)

           ;; 辅助函数：将符号链接替换为实际文件副本
           (define (dereference! path)
             (when (and (file-exists? path) (symbolic-link? path))
               (let* ((target (readlink path))
                      (resolved (if (string-prefix? "/" target)
                                    target
                                    (string-append (dirname path) "/" target)))
                      (tmp (string-append path ".tmp")))
                 (copy-file resolved tmp)
                 (delete-file path)
                 (rename-file tmp path))))

           ;; 1. 复制整个 LibreWolf 到输出（保留原权限和符号链接）
           (copy-recursively lw out)

           ;; 2. 将 zh-CN langpack 复制到 extensions 目录
           (let ((ext-dir (string-append out "/lib/librewolf/browser/extensions"))
                 (src-xpi (string-append #$librewolf-l10n-zh-cn
                                         "/lib/librewolf/browser/extensions"
                                         "/langpack-zh-CN@firefox.mozilla.org.xpi")))
             (mkdir-p ext-dir)
             (copy-file src-xpi (string-append ext-dir "/langpack-zh-CN@firefox.mozilla.org.xpi")))

             ;; 3. Patch policies.json: 将 "langpack" 加入 allowed_types，
             ;;    添加 zh-CN langpack 的强制安装策略，
             ;;    并通过 Preferences 策略显示语言切换 UI
             (let ((policies-file (string-append out "/lib/librewolf/distribution/policies.json"))
                   (xpi-url (string-append "file://" out
                                           "/lib/librewolf/browser/extensions"
                                           "/langpack-zh-CN@firefox.mozilla.org.xpi")))
               (dereference! policies-file)
               (chmod policies-file #o644)
               (substitute* policies-file
                 (("\"allowed_types\": \\[\"dictionary\", \"extension\", \"sitepermission\", \"theme\"\\]")
                  "\"allowed_types\": [\"dictionary\", \"extension\", \"langpack\", \"sitepermission\", \"theme\"]"))
               ;; 在 ExtensionSettings 中添加 langpack 的 normal_installed 策略
               (substitute* policies-file
                 (("\"uBlock0@raymondhill.net\": \\{")
                  (string-append "\"langpack-zh-CN@firefox.mozilla.org\": {\n"
                                 "                \"installation_mode\": \"normal_installed\",\n"
                                 "                \"install_url\": \"" xpi-url "\"\n"
                                 "            },\n"
                                 "            \"uBlock0@raymondhill.net\": {")))
               ;; 添加 Preferences 策略：显示语言切换 UI + 请求中文 locale
               (substitute* policies-file
                 (("\"SkipTermsOfUse\": true,")
                  (string-append "\"Preferences\": {\n"
                                 "            \"extensions.ui.locale.hidden\": {\n"
                                 "                \"Value\": false,\n"
                                 "                \"Status\": \"user\"\n"
                                 "            },\n"
                                 "            \"intl.locale.requested\": {\n"
                                 "                \"Value\": \"zh-CN, en-US\",\n"
                                 "                \"Status\": \"user\"\n"
                                 "            }\n"
                                 "        },\n"
                                 "        \"SkipTermsOfUse\": true,")))
               (chmod policies-file #o444))

           ;; 4. 修正 wrapper 脚本中硬编码的旧 store 路径
           ;;    bin/librewolf 是指向 lib/.../librewolf 的符号链接，两者都需要修正
           (for-each
            (lambda (file)
              (let ((path (string-append out "/" file)))
                (when (file-exists? path)
                  (dereference! path)
                  (chmod path #o644)
                  (substitute* path ((lw) out))
                  (chmod path #o555))))
            '("bin/librewolf"
              "lib/librewolf/librewolf"
              "share/applications/librewolf.desktop")))))
    (inputs
     (list librewolf librewolf-l10n-zh-cn))
    (native-inputs '())
    (propagated-inputs '())))
#+end_src

* Packages
** 桌面环境 相关软件包

#+NAME: desktop-packages-list
#+begin_src scheme
(cons* (specs->pkgs+out
        ;; 桌面组件
        "fuzzel"
        "mako"
        "swaylock-effects"
        "swayidle"
        "swww"
        "waypaper"

        ;; 字体
        "font-awesome"
        "font-google-noto-emoji"
        "font-nerd-fonts-iosevka"
        "font-maple-font-nf-cn"

        ;; 输入法
        "fcitx5"
        "fcitx5-lua"

        ;; 工具
        "cliphist"
        "dconf-editor"
        "dex"
        "libnotify"
        "setxkbmap"
        "wl-clipboard"
        "xprop"

        ;; XDG 相关组件
        "xdg-dbus-proxy"
        "xdg-desktop-portal-gnome"
        "xdg-desktop-portal-gtk"
        "xdg-user-dirs"
        "xdg-utils"))
#+end_src

** Emacs 相关软件包

#+NAME: emacs-packages-list
#+begin_src scheme
(cons* (specs->pkgs+out
        ;; Emacs 核心与基础
        "emacs-gcmh"
        "emacs-general"
        "emacs-no-littering"
        "emacs-pgtk"
        "emacs-spinner"
        "emacs-use-package"

        ;; 补全与迷你缓冲区
        "emacs-consult"
        "emacs-corfu"
        "emacs-corfu-terminal"
        "emacs-embark"
        "emacs-marginalia"
        "emacs-orderless"
        "emacs-vertico"

        ;; Evil 模式（Vim 模拟）
        "emacs-evil"
        "emacs-evil-collection"

        ;; 界面与外观
        "emacs-dashboard"
        "emacs-doom-modeline"
        "emacs-ef-themes"
        "emacs-kind-icon"
        "emacs-lsp-treemacs"
        "emacs-minimap"
        "emacs-nerd-icons"
        "emacs-rainbow-delimiters"
        "emacs-treemacs"
        "emacs-treemacs-extra"
        "emacs-treemacs-nerd-icons"
        "emacs-which-key"

        ;; 编辑与导航
        "emacs-ace-jump-mode"
        "emacs-diff-hl"
        "emacs-expand-region"
        "emacs-multiple-cursors"
        "emacs-origami"
        "emacs-perspective"
        "emacs-ws-butler"

        ;; 开发工具
        "emacs-apheleia"
        "emacs-eldoc-box"
        "emacs-rg"
        "emacs-stickyfunc-enhance"
        "emacs-vterm"
        "emacs-yasnippet"
        "emacs-yasnippet-snippets"

        ;; 编程语言支持
        "emacs-arei"
        "emacs-fish-mode"
        "emacs-fish-completion"
        "emacs-json-mode"
        "emacs-helm-fish-completion"
        "emacs-kotlin-mode"
        "emacs-markdown-mode"
        "emacs-nix-mode"
        "emacs-rust-mode"
        "emacs-sly"
        "emacs-typescript-mode"
        "emacs-web-mode"
        "emacs-yaml"
        "emacs-yaml-mode"
        "emacs-zig-mode"

        ;; Git 集成
        "emacs-git-messenger"
        "emacs-git-timemachine"
        "emacs-magit"
        "emacs-magit-todos"

        ;; 项目管理
        "emacs-centaur-tabs"
        "emacs-projectile"

        ;; Org Mode 生态
        "emacs-htmlize"
        "emacs-org"
        "emacs-org-appear"
        "emacs-org-modern"
        "emacs-org-roam"
        "emacs-ox-gfm"

        ;; 显示优化
        "emacs-flycheck"
        "emacs-fold-dwim"
        "emacs-helpful"
        "emacs-origami-ts"
        "emacs-posframe"
        "emacs-rainbow-delimiters"
        "emacs-symbol-overlay"

        ;; 邮件与日历
        "emacs-calfw"
        "emacs-notmuch"

        ;; 编程语言运行时与工具
        "graphviz"
        "guile-ares-rs"
        "plantuml"
        "sbcl"
        "texlive-scheme-basic"

        ;; Tree-sitter（语法解析）
        "tree-sitter"
        "tree-sitter-bash"
        "tree-sitter-c"
        "tree-sitter-cpp"
        "tree-sitter-css"
        "tree-sitter-dockerfile"
        "tree-sitter-go"
        "tree-sitter-html"
        "tree-sitter-java"
        "tree-sitter-java-properties"
        "tree-sitter-javascript"
        "tree-sitter-json"
        "tree-sitter-nix"
        "tree-sitter-python"
        "tree-sitter-rust"
        "tree-sitter-typescript"))
#+end_src

** 终端 相关软件包

#+NAME: terminal-packages-list
#+begin_src scheme
(cons* (specs->pkgs+out
        ;; 工具
        "atuin"
        "broot"
        "btop"
        "starship"
        "kitty-full"

        ;; 运行器
        "just"
        "maak"

        ;; Tmux 相关
        "tmux"
        "tmuxifier"
        "tmux-xpanes"

        ;; 基于 Rust 的命令行工具替代品
        "bat"
        "fd"
        "ripgrep"
        "fzf"
        "lolcat"
        "zoxide"))
#+end_src

** 开发环境 相关软件包

#+NAME: devtools-packages-list
#+begin_src scheme
(cons* (specs->pkgs+out
        ;; 开发环境
        "ccls"
        "clang"
        "jdtls-bin"
        "gradle"
        "maven"
        "node"
        "openjdk@25"
        "package-version-server"
        "pnpm@10"
        "python"
        "python-black"
        "python-lsp-server"
        "python-pylsp-mypy"
        "rust"
        "rust-analyzer"
        "zig"

        ;; 格式化工具
        "nixfmt"
        "python-jsbeautifier"
        "python-lsp-black"
        "shfmt"

        ;; 工具
        "bun"
        "direnv"
        "gitui"
        "git-filter-repo"
        "git-lfs"
        "jq"
        "pandoc"
        "reuse"
        "sdkmanager"
        "uv"

        ;; 编辑器
        "helix"
        "zed"))
        #+end_src
** 其他软件包

#+NAME: user-packages-list
#+begin_src scheme
(append (list librewolf-with-l10n)
  (cons* (specs->pkgs+out
        ;; 解码器
        "gstreamer"
        "gst-plugins-base"
        "gst-plugins-good"
        "gst-plugins-bad"
        "gst-plugins-ugly-full"

        ;; 影音
        "easyeffects"
        "helvum"
        "mpv"
        "nomacs"
        "obs"
        "obs-wlrobs"
        "obs-pipewire-audio-capture"
        "obs-vkcapture"

        ;; 社交
        "notmuch"
        "telegram-desktop"
        "zulip-term"

        ;; 生产力工具
        "claude-code"
        "codex"
        "gimp"
        "keepassxc"
        "git-credential-keepassxc"
        "kdeconnect"
        "seahorse"
        "virt-manager"
        "zen-browser-bin"

        ;; 娱乐
        "heroic"
        "mangohud"
        "osu-lazer-bin"
        "prismlauncher-dolly"
        "steam"

        ;; 实用工具
        "amule"
        "freerdp@3"
        "kanata"
        "pinentry-qt"
        "postgresql"
        "winapps"

        ;; 主题
        "orchis-kde-themes"
        "orchis-theme"
        "adwaita-icon-theme"
        "bibata-cursor-theme"
        "papirus-icon-theme"

        ;; QT框架相关
        "kvantum"
        "qt5ct"
        "qt6ct")))
#+end_src

* Services

#+NAME: services-modules
#+begin_src scheme
(use-modules (gnu home services desktop)
             (gnu home services dotfiles)
             (gnu home services fontutils)
             (gnu home services gnupg)
             (gnu home services shells)
             (gnu home services shepherd)
             (gnu home services niri)
             (gnu home services sound)
             (gnu home services desktop)
             (gnu home services fontutils)
             (gnu home services syncthing)
             (gnu home services guix)


             (rosenthal services desktop)
             (rosenthal services shellutils))

(use-package-modules java shells)

(use-package-modules fcitx5
                     freedesktop
                     gnome
                     gnupg
                     kde-internet
                     linux
                     polkit
                     rust-apps
                     wm
                     xdisorg)
#+end_src

** modprobe 相关服务

#+NAME: modprobe-services
#+begin_src scheme
(list ;modprobed-db 配置文件
 (simple-service 'modprobed-db-config home-files-service-type
                 `((".config/modprobed-db/modprobed-db.conf" ,(plain-file
                                                               "modprobed-db.conf"
                                                               "IGNORE=(hid-uclogic wacom)"))))

 ;; modprobed-db 激活时运行（一次性初始化）
 (simple-service 'modprobed-db-activation home-activation-service-type
                 #~(begin
                     (use-modules (guix build utils))
                     ;; 确保配置目录存在
                     (mkdir-p (string-append (getenv "HOME")
                                             "/.config/modprobed-db"))
                     ;; 运行 modprobed-db store 记录当前模块
                     (system* #$(file-append modprobed-db
                                             "/bin/modprobed-db")
                              "storesilent"))))
#+end_src

** Fish 相关服务

#+NAME: fish-services
#+begin_src scheme :noweb yes
(list (simple-service 'fish-configs
                      home-xdg-configuration-files-service-type
                      (list `("fish/conf.d/10-source.fish" ,(computed-substitution-with-inputs
                                                             "config.fish"
                                                             (plain-file
                                                              "10-source.fish"
                                                              "
<<fish-cfg>>
")
                                                             (specs->pkgs
                                                              "atuin"
                                                              "direnv"
                                                              "fzf"
                                                              "starship"
                                                              "zoxide")))))

      (simple-service 'fish-functions
                      home-xdg-configuration-files-service-type
                      (list `("fish/functions/fenv.main.fish" ,(file-append
                                                                fish-foreign-env
                                                                "/share/fish/functions/fenv.main.fish"))
                            `("fish/functions/fenv.fish" ,(file-append
                                                           fish-foreign-env
                                                           "/share/fish/functions/fenv.fish")))))
#+end_src

#+NAME: fish-cfg
#+begin_src fish
status is-interactive
and begin

    $$bin/atuin$$ init fish | source
    $$bin/direnv$$ hook fish | source
    $$bin/fzf$$ --fish | source
    $$bin/zoxide$$ init fish | source

    $$bin/starship$$ init fish | source
    enable_transience

end
#+end_src

** 桌面常用软件的相关服务

#+NAME: user-desktop-services
#+begin_src scheme
(list (service home-mako-service-type)
      (service home-syncthing-service-type)

      (service home-darkman-service-type
               (home-darkman-configuration
                (latitude 31.2304)
                (longitude 121.4737)
                (use-geoclue #f)
                (dbus-server #t)
                (portal #t)))

      (service home-shepherd-service-type
               (home-shepherd-configuration
                (auto-start? #f)
                (daemonize? #f)))

      (simple-service 'dbus home-shepherd-service-type
                      (list (shepherd-service
                             (provision '(dbus))
                             (start #~(const #t))
                             (stop #~(const #f)))))

      (service home-pipewire-service-type)

      (service home-fcitx5-service-type
               (home-fcitx5-configuration (themes (list fcitx5-material-color-theme))
                                          (input-method-editors (list fcitx5-rime))
                                          (utilities (list fcitx5-configtool fcitx5-lua))
                                          (gtk-im-module? #t)
                                          (qt-im-module? #t))))
#+end_src

** 软件自启动

这里利用 *scheme* 的 =lambda=
函数来包装了一个列表，这样的操作在我的配置文件里面多次出现，这样可以不用重复写很大一段内容
这也正是 Guix 的优势所在: 可以利用 Scheme 强大的生态来进行各种操作

#+NAME: home-shepherd-services
#+begin_src scheme
(let ((make-shepherd-service
       (lambda (pkg path requirement)
         (shepherd-service (provision (list (string->symbol (package-name pkg))))
                           (requirement requirement)
                           (start #~(make-forkexec-constructor
                                     (list #$(file-append pkg path))
                                     #:log-file (string-append (getenv "HOME") "/.var/log/"
                                                               #$(package-name pkg) ".log")))
                           (respawn? #t)))))
  (list (simple-service 'essential-desktop-services home-shepherd-service-type
                        (list (make-shepherd-service kanata "/bin/kanata" '(dbus))
                              (make-shepherd-service kdeconnect "/bin/kdeconnectd" '(dbus))
                              (make-shepherd-service polkit-gnome "/libexec/polkit-gnome-authentication-agent-1" '(dbus))
                              (make-shepherd-service poweralertd "/bin/poweralertd" '(dbus))
                              (make-shepherd-service swayidle "/bin/swayidle" '(dbus))
                              (make-shepherd-service waybar "/bin/waybar" '(dbus))
                              (make-shepherd-service xdg-desktop-portal "/libexec/xdg-desktop-portal" '(dbus))
                              (make-shepherd-service xdg-desktop-portal-gnome "/libexec/xdg-desktop-portal-gnome" '(xdg-desktop-portal))
                              (make-shepherd-service xdg-desktop-portal-gtk "/libexec/xdg-desktop-portal-gtk" '(xdg-desktop-portal))))))
#+end_src

** 计时器服务

#+NAME: timer-services
#+begin_src scheme
(list (simple-service 'auto-update home-shepherd-service-type
                      (list
                       ;; 用于自动更新flatpak中的软件包
                       (shepherd-timer '(flatpak-update)
                                       #~(calendar-event #:hours '(18)
                                                         #:minutes '(0))
                                       #~("/run/current-system/profile/bin/flatpak"
                                          "upgrade" "-y")
                                       #:requirement '(dbus)))))
#+end_src

** dotfiles 管理相关服务

*** 主要的一些内容

#+NAME: dotfile-services
#+begin_src scheme
(list (service home-dotfiles-service-type
               (home-dotfiles-configuration
                (directories '("../dotfiles"))
                (layout 'stow)
                (packages '("desktop"
                            "system"
                            "termide"
                            "terminal"
                            "themes"
                            "utilities"))
                (excluded '(".*~"
                            ".*\\.swp"
                            "\\.git"
                            "\\.gitignore"
                            "\\.github"
                            "LICENSE"
                            "README\\.md"))))

      (service home-xdg-configuration-files-service-type
               `(("gdb/gdbinit" ,%default-gdbinit)
                 ("nano/nanorc" ,%default-nanorc)))

      (simple-service 'prism-jdks home-files-service-type
                      (map (lambda (jdk)
                             (list (in-vicinity
                                    ".local/share/PrismLauncher/java/"
                                    (package-version jdk)) jdk))
                           (list openjdk25 openjdk21 openjdk17))))
#+end_src

*** 区别对待某些配置文件

有一些软件会需要在其配置文件内写入某个二进制的具体位置，此时就可以使用由
*rosenthal* 提供的 =computed-substitution-with-inputs= 函数

用法为：

1. 在需要的配置文件内以 =&&bin/foo&&= 的形式来写你所需要写入的二进制路径
2. 在 =specs->pkgs= 参数中写入能够提供这些二进制的软件包

#+NAME: home-files-services
#+begin_src scheme
(list (service home-files-service-type
               `((".guile" ,%default-dotguile)
                 (".Xdefaults" ,%default-xdefaults)
                 (".config/git-credential-keepassxc" ,(computed-substitution-with-inputs
                                                       "git-credential-keepassxc"
                                                       (local-file
                                                        "../source/files/git-credential-keepassxc")
                                                       (specs->pkgs "git"
                                                                    "fish")))
                 (".config/zed/settings.json" ,(computed-substitution-with-inputs
                                                "zed.json"
                                                (local-file
                                                 "../source/files/zed.json")
                                                (specs->pkgs
                                                 "ccls"
                                                 "clang"
                                                 "jdtls-bin"
                                                 "maven"
                                                 "node"
                                                 "package-version-server"
                                                 "python-black"
                                                 "python-jsbeautifier"
                                                 "python-lsp-black"
                                                 "python-lsp-server"
                                                 "python-pylsp-mypy"
                                                 "rust-analyzer")))))
      (service home-niri-service-type
               (home-niri-configuration (config (computed-substitution-with-inputs
                                                 "config.kdl"
                                                 (local-file
                                                  "../source/files/niri.kdl")
                                                 (specs->pkgs
                                                  "brightnessctl"
                                                  "cliphist"
                                                  "dex"
                                                  "foot"
                                                  "fish"
                                                  "fuzzel"
                                                  "niri"
                                                  "swaylock-effects"
                                                  "waypaper"
                                                  "wl-clipboard"
                                                  "xwayland-satellite"))))))
#+end_src

** 环境变量

#+NAME: 环境变量

*** 软件相关的环境变量

#+NAME: extend-environment-variables
#+begin_src scheme
'(("ANDROID_HOME" . "$HOME/Programs/Android/SDK")
  ("EDITOR" . "hx")
  ("FREERDP_ASKPASS" . "1")
  ("GUIX_PROFILE" . "$HOME/.guix-profile")
  ("GUIX_SANDBOX_HOME" . "$XDG_DATA_HOME/Sandbox")
  ("HTTP_PROXY" . "http://127.0.0.1:7890")
  ("http_proxy" . "$HTTP_PROXY")
  ("HTTPS_PROXY" . "$HTTP_PROXY")
  ("https_proxy" . "$HTTP_PROXY")
  ("LIBVIRT_DEFAULT_URI" . "qemu:///system")
  ("no_proxy" . "127.0.0.1,localhost")
  ("NO_PROXY" . "127.0.0.1,localhost")
  ("QS_ICON_THEME" . "Papirus-Dark")
  ("QT_QPA_PLATFORMTHEME" . "qt5ct")

  ;; Wayland support.
  ("GDK_BACKEND" . "wayland")
  ("_JAVA_AWT_WM_NONREPARENTING" . "1")
  ("MOZ_ENABLE_WAYLAND" . "1")
  ("QT_AUTO_SCREEN_SCALE_FACTOR" . "1"))
#+end_src

*** 为了让软件尽量遵守XDG规范而提供的变量

#+NAME: xdg-basedir-env-vars
#+begin_src scheme
'(("HISTFILE" . "$XDG_STATE_HOME/bash/history")
  ;; docker
  ("DOCKER_CONFIG" . "$XDG_CONFIG_HOME/docker")
  ;; gdb
  ("GDBHISTFILE" . "$XDG_STATE_HOME/gdb/history")
  ;; gnupg
  ("GNUPGHOME" . "$XDG_DATA_HOME/gnupg")
  ;; go
  ("GOMODCACHE" . "$XDG_CACHE_HOME/go/mod")
  ("GOPATH" . "$XDG_DATA_HOME/go")
  ;; gradle
  ("GRADLE_USER_HOME" . "$XDG_DATA_HOME/gradle")
  ;; guile
  ("GUILE_HISTORY" . "$XDG_STATE_HOME/guile/history")
  ;; luanti
  ("MINETEST_USER_PATH" . "$XDG_DATA_HOME/luanti")
  ;; node
  ("NPM_CONFIG_USERCONFIG" . "$XDG_CONFIG_HOME/npm/npmrc")
  ;; nvidia-driver
  ("CUDA_CACHE_PATH" . "$XDG_CACHE_HOME/nv")
  ;; password-store
  ("PASSWORD_STORE_DIR" . "$XDG_DATA_HOME/pass")
  ;; python
  ("PYTHON_HISTORY" . "$XDG_STATE_HOME/python/history")
  ;; rust
  ("CARGO_HOME" . "$XDG_DATA_HOME/cargo")
  ;; sqlite
  ("SQLITE_HISTORY" . "$XDG_STATE_HOME/sqlite_history")
  ;; tmuxifier
  ("TMUXIFIER_LAYOUT_PATH" . "$XDG_CONFIG_HOME/tmuxifier/layouts"))
#+end_src

*** 引用以上的所有变量，并在这里放一些多行的环境变量

#+NAME: environment-variable-services
#+begin_src scheme :noweb yes
(list (simple-service 'environment-variables
                      home-environment-variables-service-type
                      (append
                       <<extend-environment-variables>>
                       <<xdg-basedir-env-vars>>
                       `(("QT_PLUGIN_PATH" unquote
                          (string-append
                           "/run/current-system/profile/lib/qt5/plugins:"
                           "/run/current-system/profile/lib/qt6/plugins:"
                           "$HOME/.guix-home/profile/lib/qt5/plugins:"
                           "$HOME/.guix-home/profile/lib/qt6/plugins"))
                         ("CHROMIUM_FLAGS" unquote
                          (string-append
                           "--enable-features=UseOzonePlatform,WaylandWindowDecorations "
                           "--ozone-platform-hint=wayland "
                           "--enable-wayland-ime "
                           "--wayland-text-input-version=3"))
                          ("_JAVA_OPTIONS" unquote
                           (string-append
                            "-Djava.util.prefs.userRoot=$XDG_CONFIG_HOME/java "
                            "-Dawt.toolkit.name=WLToolkit"))
                          ("XDG_DATA_DIRS" unquote
                           (string-append
                            "$HOME/.nix-profile/share:"
                            "/nix/var/nix/profiles/default/share:"
                            "$XDG_DATA_DIRS"))))))
#+end_src

* Font

** 字体相关配置

#+NAME: font-services
#+begin_src scheme
(list (simple-service 'extend-fontconfig home-fontconfig-service-type
                      (list "~/.local/share/fonts"
                            "/run/current-system/profile/share/fonts"
                            (let ((sans "Sarasa Gothic SC")
                                  (serif "Sarasa Gothic SC")
                                  (mono "Maple Mono NF CN")
                                  (emoji "Noto Color Emoji"))
                              `((alias (family "sans-serif")
                                       (prefer (family ,sans)
                                               (family ,emoji)))
                                (alias (family "serif")
                                       (prefer (family ,serif)
                                               (family ,emoji)))
                                (alias (family "monospace")
                                       (prefer (family ,mono)
                                               (family ,emoji)))
                                (alias (family "emoji")
                                       (prefer (family ,emoji))))))))
#+end_src

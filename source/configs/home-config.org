...
#+NAME: sops-services
#+begin_src scheme
(list (service home-sops-secrets-service-type
               (home-sops-service-configuration
                (age-key-file (string-append (getenv "HOME") "/.config/sops/age/keys.txt"))
                (secrets
                 (list (sops-secret
                        (key '("mihomo-config"))
                        (file (local-file "../dotfiles/secrets/.config/mihomo/secrets.yaml"))
                        (path (string-append (getenv "HOME") "/.config/mihomo/config.yaml"))
                        (permissions #o600)))))))
#+end_src

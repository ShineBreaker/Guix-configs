...
+ 用户级工具、桌面环境配置、个人偏好设置应放入本文件下方的 *用户配置* 章节。
...
(list (simple-service 'fixed-machine-id etc-service-type
      ...
      ;; 在 tmpfs /home 上重建 Guix Home 环境
      (simple-service 'bootstrap-guix-home activation-service-type
                      (with-imported-modules (source-module-closure
                                             ...
                                             #~(begin
                                                 ...
                                                 (let* ((user #$username)
                                                   ...
                                                   (when (file-exists? profile-path)
                                                     ;; 1. 创建 ~/.guix-home
                                                     (let ((link (string-append home "/.guix-home")))
                                                       (unless (file-exists? link)
                                                         (symlink profile-path link)))

                                                     ;; 2. 执行 activate（重建所有 store symlink）
                                                     (catch #t
                                                            (lambda ()
                                                              (setenv "HOME" home)
                                                              (setenv "GUIX_NEW_HOME" profile-path)
                                                              (primitive-load (string-append profile-path "/activate"))
                                                              (unsetenv "GUIX_NEW_HOME"))
                                                            (lambda args
                                                              (format #t "WARNING: activate failed: ~a~%" args)))

                                                     ;; 3. 创建 ~/.guix-profile（activate 不管理此链接）
                                                     (let ((target (string-append
                                                                    "/var/guix/profiles/per-user/" user "/guix-profile"))
                                                           (link (string-append home "/.guix-profile")))
                                                     ...
                                                     (let ((target (string-append
                                                           ...
                                                           (link (string-append home "/.nix-profile")))
                                                       (when (and (file-exists? target)
                                                                  (not (file-exists? link)))
                                                         (symlink target link)))

                                                     ;; 5. Stow mutable dotfiles（emacs、pi）
                                                     ;;    stow 在 system profile 中
                                                     (catch #t
                                                            (lambda ()
                                                              (let* ((stow-bin "/run/current-system/profile/bin/stow")
                                                                     (repo (string-append home "/Projects/Config/Guix-configs"))
                                                                     (mutable-dir (string-append repo "/dotfiles/mutable")))
                                                                (when (file-exists? mutable-dir)
                                                                  (let ((pkgs (scandir mutable-dir
                                                                    ...
                                                                    (for-each (lambda (p)
                                                                                (system** stow-bin "-d" mutable-dir "-t" home "-D" p))
                                                                              pkgs)
                                                                    (for-each (lambda (p)
                                                                                (system** stow-bin "-d" mutable-dir "-t" home p))
                                                                              pkgs)))))
                                                            (lambda args
                                                              (format #t "WARNING: stow failed: ~a~%" args)))

                                                     ;; 6. chown 所有新建内容给用户（activation 以 root 运行）
                                                     (for-each (lambda (f)
                                                                 (false-if-exception (chown f uid gid)))
                                                               (find-files home (const #t)
                                                                           #:directories? #t))))))))
#+end_src

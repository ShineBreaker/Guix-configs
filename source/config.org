...
=blue build-iso= (见 blueprint.scm build-iso-command) 会 tangle 出
=tmp/live-iso.scm=, 再调 =scripts/build-image.scm= 生成 iso9660 镜像.

参考: Testament 的 config/live/*.scm; 本仓库按 gril 决策走 config.org
统一 tangle (见 docs/iso-build.md §6 / §9.4). 变体: =xfce= (主, GUI 装机辅助)
+ =minimal= (辅助, 纯 CLI fallback).

** 重要约束 (改本节必读)
- tangle 目标 =../tmp/live-iso.scm= 绝对不能复用 =../tmp/config.scm=,
  否则 =%live-installation-os= 会污染主机配置 (见 docs §9.4.5 陷阱 1).
- =<<live-modules>>= 只在本节被 =live-installation-os= 引用, 绝不能塞进
  主 =<<modules>>= 块 (会污染 tmp/config.scm).
- =blue check= 不做 cross-tangle 验证, 改完务必 =tail tmp/live-iso.scm=
  确认末行是 =%live-installation-os= (裸值).

** ISO 专用模块导入
#+NAME: live-modules
#+begin_src scheme
;; make-installation-os 在 guix core (gnu system install), 不是 rosenthal
;; (实测 source/channel.lock 锁定的 guix 9e068cc; gril fact_id=22).
;; 注: 不写 (gnu packages slim) —— 该模块不存在, slim 走 spec 解析.
(use-modules (gnu)                            ; re-export %root-account 等
             (gnu system)
             (gnu system install)            ; make-installation-os + configuration-template-service-type
             (gnu services)
             (gnu services base)             ; guix-configuration / guix-extension
             (gnu services desktop)          ; xfce-desktop-service-type
             (gnu services xorg)             ; slim-service-type / slim-configuration
             (gnu services dbus)
             ...
             (gnu packages xfce)
             ;; 注: 不写 (gnu packages rofi) —— 该模块不存在(实测 9e068cc),
             ;; rofi 在 (gnu packages xdisorg). rofi 走 spec 解析,无需 use-modules.
             (gnu packages networking)       ; mihomo (D4: 只装包不引 service)
             (nongnu packages linux)         ; linux / linux-firmware
             (nongnu system linux-initrd)
             (guix channels)
             (guix gexp)
             (guix utils)                    ; %current-system (make-installation-os #:efi-only? 用)
             (ice-9 match)
             (srfi srfi-1)
             (srfi srfi-19))
#+end_src
...
;; ---- 频道公钥 (D3: 复刻 source/config.org 主机配置的 substitute 镜像) ----
;; 装机阶段 guix pull / nonguix kernel 取 substitute 就要用这些公钥.
...
;; ---- ISO OS: XFCE 变体 (D2: 装机用环境, 不干预装好后) ----
  ...
  (operating-system
    ...
    (users
     (cons* (user-account
              (name "live")
              (group "users")
              ;; D5: 密码 = "live" —— slim auto-login 后 live user 能直接 sudo
              ;; (crypt 形式; 装机阶段不要求强密码, 装好后用户自己 passwd 改)
    ...
    (packages
     (append (specifications->packages
               '(;; CLI 工具 (installer 自身 + 装机调试)
                 ...
                 ;; 用户在 ISO 里手动 mihomo -f <user-supplied.yaml> 启动
                 "mihomo"
                 ;; XFCE 周边 (xfce-desktop-service-type 只带主包, 这里补实际用得到的)
                 "xfce4-terminal"
                 "xfce4-screenshooter"
                 "xfce4-taskmanager"
                 "thunar"
     ...
     (cons*
      ...
      (service slim-service-type
        (slim-configuration
          ...
          (auto-login? #t)
          (default-user "live")
          ;; 不设 xauth: slim-configuration 的 xauth 字段期望的是 xauth 【程序】
          ;; (默认 xauth 包), 不是 .Xauthority 文件路径. 用默认值, SLiM 自管 X 授权.
          ))

      ;; D6: kmscon 已由 make-installation-os 默认提供(install.scm:466-468,
      ;; (service kmscon-service-type (kmscon-configuration (kmscon kmscon-8) ...))),
      ;; 作为 tty1 装机回退. 不在此重复声明 —— (service kmscon-service-type)
      ;; 不带 configuration 会被 guix 拒绝("no value specified for service of
      ;; type 'kmscon'"), 而且本就冗余.

      ;; ---- 4 套 substitute 镜像 (D3: 全复刻 source/config.org 的 4 套) ----
      (simple-service 'nonguix-substitutes guix-service-type
        (guix-extension
      ...
      (modify-services (operating-system-user-services %live-base-os)
        ...
        ;;   必须用 @@ 取 (%root-account 被 (gnu) re-export 了, 这个没有).
        ;; 注 2: %default-locale-libcs 本身是 list, 要用 append 而非 cons* ——
        ;;   gc-root-service 的 builder 会 for-each symlink 每个元素, 元素必须是
        ;;   单个 store path (字符串). cons* 会把整个 list 当一个元素塞进去,
        ;;   builder 调 (symlink '("path1" "path2") "0") 会因类型错 build failed.

...
- 修复了非显而易见的 bug
- 发现了值得记录的坑/陷阱
- 完成了有参考价值 (进行了大量查询) 的任务
- 完成了需要多步 (至少十步以上) 操作的任务

写入步骤：

1. 调用 `knowledge-base` skill 获取参数规范
2. 使用 `kb add` 写入经验卡片，包含：问题背景、解决过程、关键结论
3. 若发现可复用的通用模式，使用 `kb patterns --add` 归纳

### 不必写入的情况

简单文本替换、已有经验完全覆盖的重复操作、无复用价值的一次性操作。

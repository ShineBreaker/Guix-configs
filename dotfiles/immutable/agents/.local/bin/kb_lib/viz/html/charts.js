...
function renderSidebar() {
  ...
  if (STATE.selectedCategory === null) {
    treeHtml +=
      '<li class="active" data-cat=""><span>全部</span><span class="count">' +
      CARDS.length +
      "</span></li>";
  } else {
    treeHtml +=
      '<li data-cat=""><span>全部</span><span class="count">' +
      CARDS.length +
      "</span></li>";
  }
...
function renderBarChart(svgId, data, colors, defaultH) {
  ...
  if (entries.length === 0) {
    ...
    svg.innerHTML =
      '<text x="' +
      w / 2 +
      '" y="35" fill="var(--text-dim)" font-size="11" text-anchor="middle">暂无数据</text>';
    return;
  }
  var max = entries[0][1];
  var barH = Math.max(
    10,
    Math.min(20, (defaultH - pad.t - pad.b) / entries.length),
  );
...
function renderTimeline(visible) {
  ...
  if (keys.length === 0) {
    ...
    svg.innerHTML =
      ...
      '" y="' +
      h / 2 +
      '" fill="var(--text-dim)" font-size="13" text-anchor="middle">暂无时间数据</text>';
    return;
  }
  function vals(map) {
  ...
  html +=
    '<path d="' +
    path(vU, yRight) +
    '" stroke="var(--cyan)" fill="none" stroke-width="2" stroke-dasharray="4 2"/>';
  html +=
    '<path d="' +
    path(vV, yRight) +
    '" stroke="var(--orange)" fill="none" stroke-width="2" stroke-dasharray="2 2"/>';
  ...
  for (var g2 = 0; g2 <= 4; g2++) {
    ...
    html +=
      ...
      '" y="' +
      (yy2 + 3) +
      '" fill="var(--green)" font-size="9" text-anchor="end">' +
      vl +
      "</text>";
    html +=
      '<text x="' +
...
function renderHeatmap(visible) {
  ...
  if (rows === 0) {
    ...
    svg.innerHTML =
      '<text x="300" y="35" fill="var(--text-dim)" font-size="13" text-anchor="middle">暂无技术栈热力数据（频次 ≥ 2 的 tech 不足）</text>';
    return;
  }
  ...
  months.forEach(function (m, i) {
    html +=
      ...
      '" y="20" fill="var(--text-dim)" font-size="9" text-anchor="middle">' +
      esc(m.substring(5)) +
      "</text>";
  });
  techs.forEach(function (t, ri) {
    ...
    html +=
      ...
      '" y="' +
      (yy + cellH / 2 + 3) +
      '" fill="var(--text)" font-size="10" text-anchor="end">' +
      esc(t) +
      "</text>";
    months.forEach(function (m, ci) {
      ...
      if (v > 0) {
        html +=
          ...
          '" y="' +
          (yy + cellH / 2 + 3) +
          '" fill="var(--text-bright)" font-size="9" text-anchor="middle">' +
          v +
          "</text>";
      }
    });

...
function applyGTrans() {
  ...
  g.setAttribute("transform",
    "translate(" + GCURR.tx + "," + GCURR.ty + ") " +
    "scale(" + GCURR.k + ")");
}

function setGTgt(tx, ty, k) {
  GTGT.tx = tx; GTGT.ty = ty; GTGT.k = k;
...
function animGTrans() {
  ...
  if (Math.abs(dx) < 0.1 && Math.abs(dy) < 0.1 && Math.abs(dk) < 0.001) {
    GCURR.tx = GTGT.tx; GCURR.ty = GTGT.ty; GCURR.k = GTGT.k;
    applyGTrans();
    gAnim = 0;
    return;
  }
...
function zoomGraphAt(cx, cy, factor) {
  var newK = Math.max(FORCE.ZOOM_MIN, Math.min(FORCE.ZOOM_MAX, GTGT.k * factor));
  if (newK === GTGT.k) return;
  var real = newK / GTGT.k;
  setGTgt(cx - (cx - GTGT.tx) * real, cy - (cy - GTGT.ty) * real, newK);
}

function resetGraph() { setGTgt(0, 0, 1); }

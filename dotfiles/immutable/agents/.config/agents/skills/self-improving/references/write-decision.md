...
检测到经验信号
    │
    ├─ 是技术经验吗？
    │   ├─ 排查 >2 步？ ──────── kb_mcp_kb_add_card 完整卡片
    │   ├─ 跨工具集成？ ──────── kb_mcp_kb_add_card 完整卡片
    │   ├─ 架构/设计决策？ ──── kb_mcp_kb_add_card 完整卡片
    │   ├─ 一句话修正？ ──────── kb inbox 或 kb_mcp_kb_update_card --append
    │   └─ 否 ────────────────── 不写
    │
...
| 复用价值 | 本项目                                        | 跨项目               |
| 内容量   | 1-3 行                                        | 需要章节结构         |
| 命令     | `kb inbox` / `kb_mcp_kb_update_card --append` | `kb_mcp_kb_add_card` |

...
# gate-core.sh — zcode / crush / pi 三方 gate 共享的决策核（单一真相源）
#
...
gate_bash() {
  ...
  case "$BASE" in
    cat|head|tail|bat|echo|printf|seq|date|uptime) emit AUTO_ALLOW ""; return 0 ;;
    wc|sort|uniq|tr|cut|column|rev|tac|paste|comm|diff|patch) emit AUTO_ALLOW ""; return 0 ;;
    whoami|id|hostname|uname|pwd|env|printenv|which|whereis|command|type) emit AUTO_ALLOW ""; return 0 ;;
    file|stat|realpath|readlink|basename|dirname|test|true|false) emit AUTO_ALLOW ""; return 0 ;;
    ls|tree|dust|df|free) emit AUTO_ALLOW ""; return 0 ;;
    rg|ag|ack|fd) emit AUTO_ALLOW ""; return 0 ;;
    jq|yq|mlr) emit AUTO_ALLOW ""; return 0 ;;
    dig|nslookup|host|ping|traceroute) emit AUTO_ALLOW ""; return 0 ;;
    guix)
      local word
      for word in $CMD; do
        case "$word" in
          describe|show|search|hash|lint|size|graph|weather) emit AUTO_ALLOW ""; return 0 ;;
        esac
      done
      ;;
    git)
      ...
      [[ "$git_only_read" -eq 1 ]] && { emit AUTO_ALLOW ""; return 0; }
      ;;
  esac
...
case "${1:-}" in
  bash) shift; gate_bash "$*" ;;
  edit) shift; gate_edit "${1:-}" ;;
  *)

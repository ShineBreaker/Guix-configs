...
在文档（Markdown / Org / README / docs 等）中需要流程图、架构图、时序图、状态图等示意图时：
- 一律使用 Mermaid 这类标准图表 DSL 的代码块表达，禁止手动绘制 ASCII 字符图
- 图表代码可渲染、可 diff、可维护；ASCII 图难对齐、难修改、宽度不受控
- 极简单的单行关系示意（如内联 `A -> B`）不受此限
</rule>

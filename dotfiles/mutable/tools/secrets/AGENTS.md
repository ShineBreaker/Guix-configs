...
1. **私钥绝不进 git**。私钥只能位于 `dotfiles/mutable/tools/secrets/.local/share/keys/age`
   （GNU Stow 软链到 `~/.local/share/keys/age`）。同目录 `.gitignore` 只忽略精确文件名
   `age`，**不要**放宽或删除这条规则。
2. **公钥必须入库**。`dotfiles/mutable/tools/secrets/.local/share/keys/age.pub` 由 Git 跟踪。
...
# 2. 加密 → 写入 dotfiles/mutable/tools/secrets/.local/share/secrets-encrypted/example.age
...
# 确认私钥路径与软链都正确
ls -la dotfiles/mutable/tools/secrets/.local/share/keys/age ~/.local/share/keys/age
stat -c '%a' dotfiles/mutable/tools/secrets/.local/share/keys/age     # 应为 600
...
1. **「`tools/secrets list` 报 `DECREPT_DIR: 未绑定的变量`」** —— 这是脚本里
   ...
   定,必须真跑)。
2. **「`init` 之后 `dotfiles/mutable/tools/secrets/.local/share/keys/age.pub` 是空的」** —— 脚本靠
   `awk '/^# public key:/ {print $NF}' dotfiles/mutable/tools/secrets/.local/share/keys/age > ...` 从私钥头
...
3. **「git add 时 .pub 没进暂存区」** —— 可能是误以为文件未被 ignore 但其实
   ...
   `git check-ignore -v dotfiles/mutable/tools/secrets/.local/share/keys/age.pub`(应静默返回 = 未 ignore)。
...
5. **`re-encrypt` 轮换时旧私钥丢失** —— 密钥轮换前必须先 `cp dotfiles/mutable/tools/secrets/.local/share/keys/age
...
私钥需要**手动迁移**:从旧机的 `dotfiles/mutable/tools/secrets/.local/share/keys/age` 用安全信道(物理介质 /
加密隧道)复制到新机,权限保持 600。

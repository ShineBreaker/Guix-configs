...
│   ├── atelier/           tmux 分屏 subagent 执行器 + plan-review-gate
...
| 脚本                   | 作用                                                                         |
| ---------------------- | ---------------------------------------------------------------------------- |
| `pi-xdg-runtime.sh`    | 共享 helper：初始化 XDG/PI_* 环境变量、pnpm 依赖安装、lockfile 同步 |
...
- `pi_init_xdg()` — 设置 `XDG_CONFIG_HOME` / `XDG_DATA_HOME` / `XDG_CACHE_HOME` 并 export `PI_HOME` / `PI_CODING_AGENT_DIR` / `PI_CODING_AGENT_SESSION_DIR` / `PI_OFFLINE` / `PI_PACKAGE_DIR`
- `pi_ensure_installed()` — 必要时在 `${PI_DATA_DIR}` 执行 `pnpm install`，并通过 `pi_seed_lockfile()` / `pi_sync_lockfile()` 在 stow 源与运行时之间同步 `pnpm-lock.yaml`
...
- **Pi 直接消费 `PI_HOME` / `PI_CODING_AGENT_DIR` / `PI_LOCAL_ROOT`**——所有运行时数据落在 XDG 路径下，不再有 bwrap 兼容层；如改 XDG 路径配置需验证 `pi_init_xdg()` 推导结果

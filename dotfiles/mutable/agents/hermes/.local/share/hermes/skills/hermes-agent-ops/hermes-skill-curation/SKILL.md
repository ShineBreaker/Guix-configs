...
git status -s dotfiles/mutable/agents/hermes/.local/share/hermes/skills/
```
...
cd ~/Projects/Config/Guix-configs
git status dotfiles/mutable/agents/hermes/.local/share/hermes/skills/   # 记录基线
```

### Step 5: 用 `git mv` 搬迁,处理 §1.8 未跟踪目录

```bash
cd dotfiles/mutable/agents/hermes/.local/share/hermes/skills
...
for c in <empty_category_1> <empty_category_2> ...; do
  src="dotfiles/mutable/agents/hermes/.local/share/hermes/skills/$c"
  [ ! -d "$src" ] && continue
  git rm -r "$src"

...
- [ ] **After any restructure, grep for dangling references.** When you
      ...
      anchors like "see §2.5 below", or `agenote
...
| Stale references left after a restructure (deleted `examples/x.scm` still cited; broken "§N below" internal anchors; `agenote <id>` provenance pointers; cross-skill "see skill Y" mentions) | Breaks self-containment and rot-checks; readers hit dead links or chase memory IDs that aren't inside the skill. Worst case the skill *looks* fine but silently points at deleted files. | Grep the whole skill dir for every filename/anchor you removed and every external-pointer pattern (`agenote `, other-skill names); delete or rewrite each hit. Also generify repo-specific hardcoding (see §2 "Write for the reader"). |
...
`dotfiles/mutable/agents/hermes/.local/share/hermes/skills/mlops/`):

...
| **A11: 验证对象定义域错误**                   | verifier 验的是**交付物本身**，还是源码参数 / 替身对象 / 方便测试的样本？         |

### A11 详解：验证对象定义域错误（2026-09 三次实证）

「自测绿 ≠ 现场绿」的根因几乎总是**验证对象的定义域错了一格**——verifier 在错误的定义域里证明了正确性：

| 实证 | 错在哪 | 堵法 |
| --- | --- | --- |
| 渲染器自校验脚本审「当前源码参数」而非交付的 PNG | 产物由旧参数生成也照样过校验 | 现场重渲染 + 逐像素/哈希比对交付文件 |
| usb-gate 的 E2E 用「清单口」这种方便测试的替身对象 | 证明的是「逻辑在替身上成立」，用户设备实际不在清单里 | 测试对象取自用户真实使用路径；反例出现先找定义域差异假设 |
| 用 ncurses 通道验证 webview 前端改动 | 机制通 ≠ 前端正确（两条渲染链） | 按被测界面选通道，图形前端必须走 WS 注入 + 截图 |

配套纪律：出现「自测绿 vs 用户反例」矛盾时，**第一步不是再跑一遍自测**，而是问「我验证的对象和用户用的对象是同一个吗」；
现场取证优先于重建实验（设备已拔掉也留 dmesg 拔插事件）；校验脚本自身会自相矛盾（打印 (0,0,0) 却判「一致」）——矛盾结论先怀疑测试代码。

来源: agenote 20260914-120134-5 / 20260914-121807-* / 20260912-190858

...
```
dotfiles/immutable/<app>/  →  /gnu/store/<hash>-home-dotfiles-.../  (只读副本)  →  $HOME 软链接
                          └─ Guix rebuild 时重建 ─┘
```
...
| 场景                                                          | 用哪一轨                                     |
| ------------------------------------------------------------- | -------------------------------------------- |
| 稳定配置（emacs、niri、fish）                                 | Guix stow（`dotfiles/immutable/`）           |
...
- **不要**把 `stow/` 下的任何文件加到 `dotfiles/immutable/` —— 双轨部署会产生软链接到软链接的冲突
- `home-dotfiles-service-type` 默认 `directories '("../dotfiles/immutable")`，**不**递归到 `stow/`
...
1. `feat(blue): add \`stow\` command for direct symlink deployment`——`blueprint.scm`+ 根`AGENTS.md`
2. `feat(stow): add \`<pkg>\` package for ... backing`——`stow/AGENTS.md`+`stow/<pkg>/`源文件 +`blue structor` 自动连锁更新的其他 AGENTS.md
...
- ❌ **让 `stow/` 与 `dotfiles/immutable/` 部署同一文件** —— 双链冲突，激活时哪个生效不确定

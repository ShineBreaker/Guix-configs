...
- **Check 3 originally ignored `exclude_paths` (fixed 2026-08-28).** The yaml key existed in spirit but the implementation only read `max` — a full rglob with no filtering, so whitelist entries were silently ineffective (same silent-cfg-drift class as the yaml/CHECKS parity bug). Implementation now applies `exclude_paths` via `_glob_match`; current whitelist covers `hermes-agent/.direnv/**` (nix dev-shell profile symlinks, GC-pruned, regenerated by direnv).
- **Check 14 placeholder filtering (added 2026-08-28).** Doc examples use degenerate keys like `sk-` + 20 identical chars (`sk-xxxxxxxxxxxxxxxxxxxx`) that match `[A-Za-z0-9]{20,}` and caused permanent RED on content-addressed backup blobs. The check now requires a match's alphanumeric tail (past char 6) to span more than one distinct character; real-shaped keys still go RED. Verified both directions with a throwaway real-shaped fixture (RED) and its removal (GREEN) — note the fixture must be fully purged, the scanner sweeps `.Trash-1000/` too.

- **Check 9 signature grouping must dedupe mirrored log files.** gateway.log,
  errors.log and agent.log mirror the SAME process events — file-scoped
  signatures (`{file}::{module}:{msg}`) turn one problem into three and kept
  the check permanently RED (32 "unique sigs" was really ~6). Signatures are
  now event-scoped: `module:msg` (no file prefix), one signature per event
  (an ERROR line claims its following traceback; chained inner exceptions
  httpx→httpcore→RuntimeError add nothing; a new timestamped line starts a
  new event), plus a file-level mtime pre-filter so log files without
  per-line timestamps (tui_gateway_crash.log, dashboard-restart.log) age out
  of the window with the file instead of counting forever.
- **Weixin/QQ "self-signed certificate" at gateway startup can be a
  transient race, not a MITM.** 2026-09-13: SSL_CERT_FILE/DIR pointed at the
  guix-home bundle, no proxy in the gateway's real environ, openssl
  s_client showed valid DigiCert/GlobalSign chains on both endpoints, all
  50 errors of the day clustered within 30s of gateway start (22:14:31
  boot, 22:15:20-25 errors, then clean for an hour). Cross-validate the

...
2. **写入串行**：同一时间只有一个 worker 写入，异步 worker 写入时主会话不编辑同区域。
3. **任务粒度**：每个 subagent 任务应该是单一、有明确产出物的契约。
4. **上下文隔离**：研究员和审阅员用 `context: "fresh"` 避免污染。
5. **完整上下文**：给 subagent 的任务要包含完整上下文（目标、约束、验证标准），不要只简单转发指令。
6. **worker 必须并发**：拆成 N 个独立子任务，用 `tasks: [...]` 数组并行调用。N=1 也合法（仍走 parallel 路径）。`single: { agent: "worker" }` 会被框架拒绝。
7. **worker 紧急 override**：紧急情况允许 single worker，30s 内重试一次相同的 `single` 调用视为 override 放行（每 30s 冷却一次）。

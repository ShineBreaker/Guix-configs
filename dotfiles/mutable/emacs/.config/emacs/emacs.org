...
:END:
Knowledge 与 agenote 共用的同步进程入口。统一过滤 nil 参数并返回 =(STATUS . OUTPUT)= ,避免功能模块各自维护相同实现。
...
*** 全局键描述
#+begin_src emacs-lisp :noweb-ref appearance/which-key-global :tangle no
...
*** Major mode 局部描述
#+begin_src emacs-lisp :noweb-ref appearance/which-key-major-modes :tangle no
...
*** 正则替换规则
#+begin_src emacs-lisp :noweb-ref appearance/which-key-regexp :tangle no
...
#+begin_src emacs-lisp
<<appearance/which-key-global>>
<<appearance/which-key-major-modes>>
<<appearance/which-key-regexp>>
#+end_src
...
*** 翻译数据
#+begin_src emacs-lisp :noweb-ref appearance/context-menu-data :tangle no
...
+ *context-menu* : 对 =context-menu-map= 做 advice,翻译右键菜单英文项(~930 行,见 =appearance/context-menu= 数据块)
+ *which-key 数据* : 集中维护 C-x / C-c / M-s / M-g 等前缀子树的中文描述,数据在 =appearance/which-key-data= 数据块(~1080 行)
...
;; Which-key 中文描述注册(展开 =appearance/which-key-data= 数据块)
...
*** Emacs 内知识库视图
#+begin_src emacs-lisp :noweb-ref org/knowledge-viz :tangle no
...
*** 浏览视图
#+begin_src emacs-lisp :noweb-ref org/agenote-view :tangle no
;; 面板状态（buffer-local）

...
(defun literal--eglot-setup-once ()
  ...
  ;; 不能在这里直接访问 eglot-server-programs / eglot--lookup-mode
  ;; (eglot 未加载时它们是 void / void-function)。
...
;; 注册语言服务器程序表 + 为可用语言挂 eglot-ensure。
(with-eval-after-load 'eglot
...
(use-package projectile
  ...
  ;; projectile alien 模式按 \0(空字节)切分输出(projectile.el 的
  ;; projectile-files-via-ext-command),故每条命令必须输出 null 分隔。
  ;; 注意:fd 的短选项是 -t,不接受 -type(会把 "-type" 当 --type 解析,
  ;; 吞掉 "ype" 报错 "invalid value 'ype'");必须用长选项 --type f。
  ;; rg 分支同样需要 --null,否则整个换行分隔输出被当成单个文件名。
  (setq projectile-generic-command
        (cond
         (literal:executable-fd
          (concat literal:executable-fd
                  " . --type f --color=never -0"
                  " --strip-cwd-prefix --hidden --follow --exclude .git"))
         (literal:executable-rg
          (concat literal:executable-rg
                  " --files --hidden --follow --color=never --null -g '!.git'"))

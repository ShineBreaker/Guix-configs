...
#+begin_src emacs-lisp :noweb-ref appearance/which-key-data :tangle no
...
#+begin_src emacs-lisp
<<appearance/which-key-data>>
#+end_src


#+begin_src emacs-lisp :noweb-ref appearance/context-menu :tangle no
...
#+begin_src emacs-lisp :noweb-ref appearance/help :tangle no
...
#+begin_src emacs-lisp :noweb-ref appearance/modeline :tangle no
...
#+begin_src emacs-lisp :noweb-ref appearance/tab-line :tangle no
...
#+begin_src emacs-lisp :noweb-ref appearance/git :tangle no
...
#+begin_src emacs-lisp :noweb-ref appearance/color-scheme :tangle no
...
+ =literal/knowledge-*= :知识库经验卡片 CRUD(~980 行)
+ =literal/agenote-*= :agenote CLI 集成(~300 行,见后文 =** agenote= 章节)
...
#+begin_src emacs-lisp :noweb-ref org/knowledge :tangle no
...
#+begin_src emacs-lisp :noweb-ref org/agenote :tangle no
...
+ *自包含插件化*: 模块零跨模块 require,通过 3 个 defvar 注入点接收依赖:
  - =literal:executable-aspell= / =literal:executable-hunspell= ← bootstrap 的 defconst 自动生效(机制 1,defvar 不覆盖已 bound 值)
  - =literal/add-frame-hook= ← 已在 *Frame 生命周期* 块注入实现(机制 2,Lisp-2 双命名空间);nil 时 per-frame 的 GUI/终端 childframe 适配跳过
...
#+begin_src emacs-lisp :noweb-ref keys/completion :tangle no
...
;;
;; 自包含设计（遵循 AGENTS.md 插件化规范）：
;; 本模块不 require 任何 literal-* 模块。以下符号通过 defvar 注入点由启动域
;; 提供真实实现；未注入时模块仍可加载，仅对应功能降级：
;;
;; 注入点 1（路径常量，AGENTS.md 机制 1，defvar 不覆盖已 bound 值）：
;;   - `literal:executable-aspell'   ← 启动与基础设施域 defconst 自动生效
;;   - `literal:executable-hunspell' ← 启动与基础设施域 defconst 自动生效
;;   未注入（nil）时：ispell 后端检测跳过，cape 补全不受影响。
;;
;; 注入点 2（frame 生命周期 hook）：
;;   - `literal/add-frame-hook' ← 由启动与基础设施域的 Frame 块定义(同 main.el 内更早位置),
;;     per-frame 的 GUI/终端 childframe 适配由其提供实现。
;;
...
(defvar literal:executable-aspell nil
  "aspell 可执行文件路径。由启动与基础设施域 defconst 自动生效。
nil 时 ispell 后端检测跳过。")

(defvar literal:executable-hunspell nil
  "hunspell 可执行文件路径。由启动与基础设施域 defconst 自动生效。
nil 时 ispell 后端检测跳过。")

;; frame hook 由启动与基础设施域的 Frame 块提供(更早位置已加载)
...
注入点（由启动与基础设施域 defconst 自动生效）。两者均 nil 时为 no-op。"
...
;; 已落在 keys/completion 数据块内,本块仅保留 FIXME 追踪记录。
;; 等上游修复后,在 keys/completion 的 vertico-multiform-categories 里
...
Dashboard 因依赖知识库 / 帮助 / 颜色方案 / frame 模块的函数,延后到此处加载(所有依赖之后)。

+ Dashboard 模块(~1000 行)直接 require 依赖模块(frame / knowledge / help / color-scheme),内部调用对应函数(带 fboundp 守卫)
...
#+begin_src emacs-lisp :noweb-ref system/dashboard :tangle no

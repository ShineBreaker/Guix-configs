...
;; - `literal:org-directory' / `literal:org-inbox-file'（路径，下方 defvar）
;; - `literal/add-frame-hook'（frame 生命周期，下方 defvar）
;; - 4 个回调注入点（知识库卡片 / 文件打开 / 快捷键提取 / 主题刷新注册）
;;   未注入时对应卡片为空或刷新跳过，dashboard 仍可独立加载。
...
;; ═════════════════════════════════════════════════════════════════════════════
;; 注入点：路径、frame hook、跨模块回调（均由 init.el 注入）
;; ═════════════════════════════════════════════════════════════════════════════
(defvar literal:org-directory nil
  "Org 文件根目录。由 init.el 注入；nil 时知识库卡片回退到默认行为。")
(defvar literal:org-inbox-file nil
  "通用收件箱文件路径。由 init.el 注入。")
(defvar literal/add-frame-hook nil
  "注册函数在每个新 frame 创建时执行的 hook。
由 init.el 注入 literal-frame.el 的实现。nil 时 per-frame 接管跳过。
函数签名：(FUNCTION &optional FRAME)。")

(defvar literal/dashboard-collect-knowledge-files-fn nil
  "收集知识库 org 文件的函数（init.el 注入 literal/knowledge--collect-org-files）。
nil 时知识库「最近条目」卡片扫描目录时直接用 `directory-files'。")
(defvar literal/dashboard-open-knowledge-file-fn nil
  "打开知识库文件的函数（init.el 注入 literal/knowledge-open-file）。
nil 时知识库按钮回退到 `find-file'。")
(defvar literal/dashboard-extract-bindings-fn nil
  "提取快捷键展示数据的函数（init.el 注入 literal/help--extract-dashboard-bindings）。
nil 时快捷键卡片为空。")
(defvar literal/dashboard-register-refresh-fn nil
  "注册主题切换 buffer 刷新的函数（init.el 注入 literal/register-buffer-refresh!）。
nil 时 dashboard buffer 不参与主题切换刷新。")
         ...
         (rows (if entries
                   (mapcar (lambda (e)
                             (let* ((path (car e))
                               ...
                               (list (literal/dashboard--make-item-button
                                      display path
                                      (or literal/dashboard-open-knowledge-file-fn
                                          #'find-file)))))
                           entries)
                 '(("知识库暂无条目")))))
    (literal/dashboard--card-section
...
(defun literal/dashboard--extract-top-level-bindings ()
  ...
  (when literal/dashboard-extract-bindings-fn
    (funcall literal/dashboard-extract-bindings-fn)))
...
(use-package dashboard
  ...
  (when (functionp literal/dashboard-register-refresh-fn)
    (funcall literal/dashboard-register-refresh-fn
...
(when (functionp literal/add-frame-hook)
  (funcall literal/add-frame-hook #'literal/dashboard-open-for-client-frame))

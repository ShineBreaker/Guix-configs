...
* 启动顺序与模块依赖速查
本配置 =emacs.org= 由 8 个顶层域构成(末尾 =* init.el ends here= 为占位),每个域直接内联定义对应的逻辑模块,通过连续的 src 块组织。模块间通过加载顺序表达依赖(见 ADR-0003)。

+ 路径常量与外部命令缓存在 =启动与基础设施= 域内联定义,后续所有模块直接引用
+ Frame hook 实现在 =启动与基础设施= 域内联定义;color-scheme / dashboard / completion 直接调用 =literal/add-frame-hook=
+ Dashboard 直接调用 knowledge / help / color-scheme / frame 的函数(带 fboundp 守卫);延后到 =* 系统工具与实验性= 域末尾加载以等所有依赖就绪

| 加载顺序 | 顶层域           | 内联逻辑模块                                                                                                              | 关键跨域依赖点                                                                                                                                                                                                 |
|----------+------------------+---------------------------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
|        3 | 编辑与文本       | (无外置模块)                                                                                                              | 本域内联定义 =literal/{copy,cut,rename,comment}-dwim= 等编辑命令; =delete-selection-mode= 覆盖选区粘贴; =with-eval-after-load which-key= 注册中文描述 |
|        4 | 编程与开发       | (无新增模块,见 #2)                                                                                                        | 共享 §2 已加载的 git / color-scheme / tab-line / modeline / help                                                                                                                                                |
...
注:行 2 「界面与外观」罗列的 7 个模块由多个 src 块组成(§启动与基础设施 内联 frame,§界面与外观 内联 git/color-scheme/tab-line/modeline/help/context-menu/which-key-data),跨域语义各自独立。
...
#+begin_src emacs-lisp :noweb-ref module/literal-which-key-data :tangle no
...
#+begin_src emacs-lisp
<<module/literal-which-key-data>>
#+end_src


#+begin_src emacs-lisp :noweb-ref module/literal-context-menu :tangle no
...
;;
;; 维护方式：
;; - 常见右键项写入 `literal:context-menu-label-translations'
...
(defconst literal:context-menu-label-translations
  '(("Context Menu" . "右键菜单")
    ("Undo" . "撤销")
    ("Undo in Region" . "撤销选区")
    ("Redo" . "重做")
    ...
    ("Run checker as compile command" . "以编译命令运行检查器")
    ...
    ("React" . "回应")
    ("Add reaction" . "添加回应")
    ("Remove reaction" . "移除回应")
    ("View source" . "查看源文本")
    ("View raw" . "查看原文")
    ("Copy message link" . "复制消息链接")
    ("Copy link" . "复制链接")
    ("Copy matrix.to link" . "复制 matrix.to 链接")
    ("Copy text" . "复制文本")
...
#+begin_src emacs-lisp
<<module/literal-context-menu>>
#+end_src


#+begin_src emacs-lisp :noweb-ref module/literal-help :tangle no
...
#+begin_src emacs-lisp :noweb-ref module/literal-modeline :tangle no
...
#+begin_src emacs-lisp
<<module/literal-modeline>>
#+end_src


#+begin_src emacs-lisp :noweb-ref module/literal-tab-line :tangle no
...
#+begin_src emacs-lisp
<<module/literal-tab-line>>
#+end_src


#+begin_src emacs-lisp :noweb-ref module/literal-git :tangle no
...
** Which-key 数据(模块内联:literal-which-key-data)

由 emacs.org tangle 时将 =lisp/literal-which-key-data.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。


** Tab-line(模块内联:literal-tab-line)

由 emacs.org tangle 时将 =lisp/literal-tab-line.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。


** Modeline(spaceline)(模块内联:literal-modeline)

由 emacs.org tangle 时将 =lisp/literal-modeline.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。


** Help 系统(模块内联:literal-help)

由 emacs.org tangle 时将 =lisp/literal-help.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。


** Git 操作与 display-buffer 路由(模块内联:literal-git)

由 emacs.org tangle 时将 =lisp/literal-git.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。


** Context menu(右键菜单)(模块内联:literal-context-menu)

由 emacs.org tangle 时将 =lisp/literal-context-menu.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。


** 颜色方案(ef-themes)(模块内联:literal-color-scheme)

由 emacs.org tangle 时将 =lisp/literal-color-scheme.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。
...
(defun literal/adjust-line-number-width ()
  ...
  (when (and (boundp 'display-line-numbers-width)
    ...
    (let* ((max-line (line-number-at-pos (point-max) 'absolute))
      ...
      ;; diff-hl 用 fringe 显示 Git 差异、flycheck-indication-mode 设为
...
;; ═════════════════════════════════════════════════════════════════════════════
;; lisp/ 子目录已不存在;Which-key 数据模块直接由 emacs.org tangle 时嵌入 main.el。
...
** Git 操作与 display-buffer 路由

由 emacs.org tangle 时将 =lisp/literal-git.el= 内容嵌入 main_el。


** 颜色方案(ef-themes)

由 emacs.org tangle 时将 =lisp/literal-color-scheme.el= 内容嵌入 main_el。


#+begin_src emacs-lisp :noweb-ref module/literal-color-scheme :tangle no
...
#+begin_src emacs-lisp
<<module/literal-color-scheme>>
#+end_src


** Tab-line

由 emacs.org tangle 时将 =lisp/literal-tab-line.el= 内容嵌入 main_el。


** Modeline(spaceline)

由 emacs.org tangle 时将 =lisp/literal-modeline.el= 内容嵌入 main_el。


** Help 系统

由 emacs.org tangle 时将 =lisp/literal-help.el= 内容嵌入 main_el。


** Context menu(右键菜单)

由 emacs.org tangle 时将 =lisp/literal-context-menu.el= 内容嵌入 main_el。


** Which-key 数据

由 emacs.org tangle 时将 =lisp/literal-which-key-data.el= 内容嵌入 main_el。
...
;; 本块用 cl-find-if / cl-set-difference,显式 require cl-lib(原靠其他包间接加载)。
(require 'cl-lib)
...
(use-package flycheck
  ...
  ;; flycheck-error-list-mode-map 定义在 flycheck.el 本体里,没有独立的
  ;; (provide 'flycheck-error-list) feature;此处 :config 执行时 flycheck 已加载,
  ;; keymap 已存在,直接绑定即可(原先用 with-eval-after-load 'flycheck-error-list
  ;; 永不触发,j/k/q/RET 全是死代码)。
...
;; 用官方支持的 eglot-stay-out-of:顶层 remove-hook 带 LOCAL=t 只在"当前 buffer"
;; 里移除,顶层执行时无目标 buffer,等于空操作,eglot 管理的每个 buffer 仍会挂 flymake。
(with-eval-after-load 'eglot
...
;; Scheme (Guile) + Arei
;;
;; arei 0.9.7 的 .elc(Guix 用旧版 Emacs byte-compile)与 Emacs 31 的
;; `buffer-local-set-state' 宏不兼容:arei-mode 第 356 行调用该宏,但旧 .elc
;; 把它当函数调用,参数 (sesman-system 'Arei) 被整体传给
;; buffer-local-set-state--get → wrong-type-argument (symbolp 'Arei)。
;; 此错误在 scheme-mode-hook 里抛出,会中断整个 scheme-mode 初始化,
;; 导致 font-lock 失效 → Scheme 代码不染色。
;; 修复:从 .el 源加载 arei(绕过坏的 .elc);并对 arei-mode 调用加
;; condition-case 守护,确保 minor-mode 故障不拖垮 major-mode 的 font-lock。
...
** Knowledge 系统(模块内联:literal-org-knowledge)

由 emacs.org tangle 时将 =lisp/literal-org-knowledge.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。


** agenote(模块内联:literal-org-agenote)

由 emacs.org tangle 时将 =lisp/literal-org-agenote.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。
...
由 emacs.org tangle 时将 =lisp/literal-org-knowledge.el= 内容嵌入 main_el。


#+begin_src emacs-lisp :noweb-ref module/literal-org-knowledge :tangle no
...
#+begin_src emacs-lisp
<<module/literal-org-knowledge>>
#+end_src


** agenote

由 emacs.org tangle 时将 =lisp/literal-org-agenote.el= 内容嵌入 main_el。


#+begin_src emacs-lisp :noweb-ref module/literal-org-agenote :tangle no
...
#+begin_src emacs-lisp
<<module/literal-org-agenote>>
#+end_src

* 键位与补全
全局直达键 + 多前缀体系(C-x / C-c / M-s / M-g 等)+ which-key 本体加载 + helpful 增强帮助 + winner 窗口布局撤销 + 补全框架全家桶(Vertico / Corfu / Consult)。

** Completion 框架(模块内联:literal-completion)

由 emacs.org tangle 时将 =lisp/literal-completion.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。
...
+ =literal/tabs-next= / =literal/tabs-previous= / =literal/tabs-refresh-context= 由外置 =literal-tab-line.el= 提供(本块不再重复定义):在 per-frame buffer 序列中按 ARG 步数取模循环,经 =literal--tabs-switch-to-buffer= 切换
...
;; 生命周期管理(agenote CRUD,外置 literal-org-knowledge.el 提供)
...
由 emacs.org tangle 时将 =lisp/literal-completion.el= 内容嵌入 main_el。


#+begin_src emacs-lisp :noweb-ref module/literal-completion :tangle no
...
#+begin_src emacs-lisp
<<module/literal-completion>>
#+end_src

* 系统工具与实验性
Guix 环境检测、daemon 预热、Dashboard 回调注入、Emacs 31 兼容性 hack、需要决策的配置。

** Dashboard(模块内联:literal-dashboard)

由 emacs.org tangle 时将 =lisp/literal-dashboard.el= 内容嵌入 main.el。
本节位于顶层域顶部,以保证同域下游子树能直接调用本模块导出的函数。


** Dashboard 加载
Dashboard 因依赖知识库 / 帮助 / 颜色方案 / frame 模块的函数,延后到此处加载(所有依赖之后)。原 defvar 回调注入点已改为直接 require(见 ADR-0002),此处只需 require dashboard。

+ Dashboard 模块( =lisp/literal-dashboard.el= ,~1000 行)直接 require 依赖模块(frame / knowledge / help / color-scheme),内部调用对应函数(带 fboundp 守卫)
+ 此处加载顺序保证: knowledge / help / color-scheme / frame 已加载(分别在 *Org 与知识库* / *界面与外观* / *启动与基础设施* 域)
+ dashboard 调用的跨模块函数:
...
#+begin_src emacs-lisp
;; ═════════════════════════════════════════════════════════════════════════════
;; 加载外置 Dashboard 子系统(依赖 knowledge/help/color-scheme/frame 已加载)
;; dashboard 内部直接 require 这些模块并调用对应函数(ADR-0002)
;; ═════════════════════════════════════════════════════════════════════════════
#+end_src
...
由 emacs.org tangle 时将 =lisp/literal-dashboard.el= 内容嵌入 main_el。


#+begin_src emacs-lisp :noweb-ref module/literal-dashboard :tangle no

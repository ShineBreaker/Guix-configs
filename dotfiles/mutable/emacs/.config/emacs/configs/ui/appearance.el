...
(dolist (param `((menu-bar-lines . 0)
                 ...
                 (alpha-background . ,custom:frame-background-opacity)))
...
当 `custom:frame-background-opacity' < 100 时，背景和 box 设为
unspecified 以跟随 frame 透明度，仅靠前景色和下划线区分标签。"
  (let* ((valid-bg (lambda (val) (and val (not (string-prefix-p "unspecified" val)) val)))
         ...
         (default-height (let ((h (face-attribute 'default :height nil t)))
                           (if (integerp h) h 120)))
         (vpad (max 2 (round (/ default-height 40.0))))
         (transparent-p (< custom:frame-background-opacity 100)))
    (if transparent-p
        ;; ── 透明模式：背景全部 unspecified，靠前景色 + 下划线区分 ──
        (progn
      ...
      (set-face-attribute 'tab-line-tab-current nil
                          ...
                          :foreground default-fg
                          :box (list :line-width vpad :color default-bg)
                          :weight 'medium
                          :underline `(:color ,accent-fg :position t))
      (set-face-attribute 'tab-line-tab nil
                          :background mode-line-bg
                          :foreground default-fg
                          :box (list :line-width vpad :color mode-line-bg)
                          :weight 'medium)
      (set-face-attribute 'tab-line-tab-inactive nil
                          :background mode-line-inactive-bg
                          :foreground shadow-fg
                          :box (list :line-width vpad :color mode-line-inactive-bg))
      (set-face-attribute 'tab-line-highlight nil
                          :background mode-line-bg
                          :foreground default-fg
                          :box (list :line-width vpad :color mode-line-bg))

...
本目录（`dotfiles/mutable/emacs/.config/emacs/`）顶层是 **chemacs2 引导层**，部署后占据 `~/.config/emacs/`，是 Emacs 启动的真正入口。
...
# 切默认到新配置（稳定后）
echo literal > dotfiles/mutable/emacs/.config/chemacs/profile
blue stow --restow emacs
herd restart emacs-daemon

# 改旧配置源（submodule，改源即生效，无需 stow）
# 直接编辑 dotfiles/mutable/emacs/.config/emacs/general-config/ 下的文件
```

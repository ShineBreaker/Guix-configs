...
| 命令         | 策展用途                                     |
| ------------ | -------------------------------------------- |
| `kb stats`   | 知识库统计概览（卡片数、分布、时间范围）     |
| `kb connect` | 为有隐含关联的卡片建立双向链接               |
| `kb update`  | 更新卡片属性或追加勘误                       |
| `kb inbox`   | 从对话中快速捕获待处理的点子                 |
...
4.  **缺失关联**：卡片之间有隐含联系但无互相引用 → 用 `kb connect` 建立双向链接
5.  **双时态检查**：同一主题有多张卡片但未标注演化关系 → 补充时间标注和替代关系
...
- 新卡片推翻旧结论 → 用 `kb update` 在旧卡片追加勘误，不覆盖旧内容

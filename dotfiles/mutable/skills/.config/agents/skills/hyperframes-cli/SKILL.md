...
**Recurring failure-mode fixes** (compiled from real sessions — read the matching one the first time you hit a gate error or a Guix/Nix install issue, BEFORE re-authoring scenes):

- `references/lint-validate-inspect-recipes.md` — every common lint/validate/inspect error and the minimal fix (the `class="clip"` trap, font whitelist, GSAP overlap, layout collapse, doctor's "harmless" warnings)
- `references/guix-npm-install.md` — Guix/Nix `npm install -g` ENOENT fix, the two-line workaround that puts the global install under `~/.npm-global`

...
<blueprint
  id="structural-eval-tree"
  path="references/blueprints/structural-eval-tree.md"
  example="examples/structural-eval-tree.html"
  role="concept-demo"
  duration="14-22s"
  phases="5"
  uses="svg-icon-enrichment, svg-path-draw, orbital-pulse"
  triggers="recursion visualized, syntax tree evaluation, expression tree, call stack, dependency graph, protocol handshake, data flow, control flow graph, AST walk">
An SVG tree (root + edges + leaves) builds itself; a colored pulse travels root→leaf→root to show recursive evaluation; leaf state colors change as the pulse passes. Example (18s, evaluating `(+ 1 2)`): phase 1 source code appears on the left → phase 2 root node spawns from source position with `back.out(1.7)` → phase 3 edges draw via `stroke-dashoffset` and leaves fly in from the source line with a single shared tween → phase 4 root pulses green, two particles ride the edges (`tl.to({ attr: { cx, cy } })` with `power2.inOut`) into the leaves which turn green when the particle arrives → phase 5 root shifts to amber, a result pill on the left ticks up `0 → 3` (just appears with `back.out(1.4)` + breathing box-shadow) and the whole tree scales 1.04 with yoyo. Demonstrates `attr` GSAP tweening (cx/cy/stroke-dashoffset/stroke-dasharray), particle-marker-along-edge pattern (no MotionPath plugin needed), and the "the visual IS the explanation" principle from `hyperframes-creative` → Structural visual. Use this for any concept where the rule is a process that visits nodes in a known order: AST evaluation, TCP handshake, mitosis phase order, BFS/DFS walk, dependency resolution, recursive descent parsing. See `references/structural-visual-catalog.md` for the full set of structures this template extends to.
</blueprint>
</blueprints>

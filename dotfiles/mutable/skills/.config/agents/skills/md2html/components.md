...
<a href="#deep-subsection-id" class="lvl-4">Sub-sub-section (rare)</a>
```

- One `<a>` per H2/H3 in the document.
- The `href` must match an `id="..."` on the target heading: `<h2 id="section-id">...</h2>`.
- Use `class="lvl-2"` for top-level (H2), `class="lvl-3"` for nested (H3), `class="lvl-4"` for H4 deep-nests.
- `lvl-4` is supported but should be used sparingly — long technical documents (manuals, books, RFCs) often justify it; short plan/spec docs typically do not. When in doubt, drop H4 entries from the TOC and keep them in the body only.
...
## 15. Pandoc-bridge — when NOT to hand-rebuild the body

The standard md2html workflow is to hand-rebuild the body using components from §1-§14. For **medium-to-large Markdown sources** (50 KB – 5 MB, hundreds of headings, dense code blocks), hand-rebuilding becomes impractical — you'll spend hours on plumbing the AI never sees again.

A "pandoc bridge" uses pandoc as a structural translator and lets the boot script add the polish (TOC scrollspy, anchor inject, copy buttons):

### 4 steps

1. **Convert MD → HTML body** with pandoc:

   ```sh
   pandoc --from=gfm+footnotes --to=html5 \
     --section-divs --wrap=none \
     -o body.html source.md
   ```

   This produces `<section id="..."> ... <hN>...</hN> ... </section>` markup with one `id` per section and `<pre class="language-xxx"><code>...</code></pre>` for code blocks.

2. **Move section-level IDs onto the inner heading** so the template's anchor-injection can find them:

   ```python
   import re
   body = open("body.html").read()
   # Walk every <section id="X" class="levelN">  →  <section class="levelN"><hN id="X">...</hN>
   pat = re.compile(r'<section id="([^"]+)" class="(level\d+)">')
   out, i = [], 0
   while m := pat.search(body, i):
       out.append(body[i:m.start()])
       out.append(f'<section class="{m.group(2)}">')
       rest = body[m.end():]
       hm = re.match(r'<(\s*)(h[1-6])((?:\s+[^>]*)?)>', rest)
       if hm and 'id=' not in hm.group(0):
           full = f'<{hm.group(1)}{hm.group(2)} id="{m.group(1)}"{hm.group(3)}>'
           out.append(full)
           i = m.end() + len(full) - len(rest)
           # (use len(full) / careful rewrite; see hermes/scripts/build_md2html.py for a working variant)
       else:
           out.append(rest); i = len(body)
   ```

3. **Trim irrelevant top-level matter** (front-matter H1, generator boilerplate). Keep `<section>` blocks but drop the ones before the first real chapter H1.

4. **Inject into template**, replacing `<!-- CONTENT_START --> ... <!-- CONTENT_END -->` (and optionally the new `{{AUDIT_NOTES}}` slot for translator-style caveats above the body).

### What you do NOT get with this path

- ❌ No selective component wrapping (no callouts, no step-cards, no mermaid diagrams). If the source has them as `> **Note:** ...` blockquotes, they stay as blockquotes.
- ❌ No priority-based component selection (the §11 cheatsheet only applies to hand-rebuilt sections).

### What you DO get

- ✅ Every heading has `id`, so `injectAnchors()` + `setupTocSpy()` work.
- ✅ Every code block stays `<pre><code>`, so `injectCopyButtons()` works.
- ✅ Tables, blockquotes, lists pass through unchanged.
- ✅ Editor fonts/code highlighting passthrough via `class="language-..."`.
- ✅ A document that opens in a browser, navigable by TOC, in minutes instead of hours.

### When to choose pandoc-bridge vs. hand-rebuild

| Source size        | Hand-rebuild      | Pandoc-bridge |
| ------------------ | ----------------- | ------------- |
| < 5 KB / < 30 lines| ✅ trivial         | Overkill       |
| 5 – 50 KB          | ✅ usually fast    | OK if you trust the layout  |
| 50 – 500 KB        | ⚠️ 10× more work  | ✅ recommended |
| > 500 KB           | ❌ impractical     | ✅ required    |

The cost of hand-rebuild is linear in length; the value is component-level polish. Past ~50 KB, polish stops being visible. Past ~500 KB, hand-rebuild becomes a multi-hour tax.

### Reflow caveat

When the body comes from pandoc, **every heading has an anchor link** in the UI. Some sources have a single H1 repeated for chapters — that becomes 100+ anchor icons. If that feels noisy, hide them via:

```css
.content h4 .anchor { display: none; }
```

(hand-edit in the output, or post-process with a small CSS injection.)

---

## Appendix · Future-proofing checklist

When updating `template.html`:

- ✅ Add new icon IDs to the `<svg class="icon-sprite">` block before any component references them. The boot script does not validate icon presence; missing sprites render blank.
- ✅ Add new interaction functions (e.g. `setupXxx`) AND register them inside the `DOMContentLoaded` boot block.
- ✅ Add the resource's print-hiding rule to `@media print { ... }`.
- ✅ If adding a new placeholder `{{X}}`, document it in `SKILL.md` step 3.

When updating `components.md`:

- ✅ Match snippet CSS classes verbatim — the template styles are scoped by exact class names.
- ✅ One logical component per snippet, even if it spans visual variants.
- ✅ New headings (`### 4x.`) become table of contents entries in the components index.

...
2. **Replace placeholders** in the template (all values come from Step 2 analysis, language-matched):
   ...
   - `{{AUDIT_NOTES}}` → **optional**. Injected right after `<!-- CONTENT_START -->` (before the rendered body). Use for translator / reviewer caveats, version-drift warnings, or any callout that should sit above the actual content. Leave empty string if unused.

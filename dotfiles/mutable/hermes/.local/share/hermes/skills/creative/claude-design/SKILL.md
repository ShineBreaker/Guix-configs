...
- **interactive teaching / learning sites** built from long-form source material (tutorial markdown, course notes, documentation). See the "Educational Variant" section below.
...
- "重新制作为网页", "做成互动教程", "适合初学者", "加上名词解释", "做成可互动的网站", "turn this into a website", "make it interactive", "teaching website for beginners"
- source material is **long-form instructional content** with sections/chapters/day-by-day layout, code blocks, terminology that beginners would not know
- the user explicitly wants interactivity, animations, hover/click behavior, progress tracking, or "more engaging than a markdown page"

### What it is NOT

- not a marketing landing page (no funnel, no CTA, no pricing)
- not a design system preview (the audience is end learners, not designers)
- not a SaaS dashboard (no live data, no auth, no CRUD)
- not a slide deck (it is scrollable, searchable, persistent across sessions)

### Key shape differences from a marketing artifact
...
### Anti-slop addendum for teaching sites

The general anti-slop list still applies, plus:

- **do not** wrap every section in a card with left-border accent color — use plain h2/h3 with border-bottom separator instead. Real documentation is not SaaS cards.
- **do not** add "🎉 Congratulations!" badges on every section — reserve celebration moments for genuine wins (e.g. completing the last day).
- **do not** generate fake learner counts, fake testimonials, fake engagement metrics.
- **do** include keyboard shortcut hints as inline `<kbd>` chips — this is what beginners actually need.
- **do** expose every term a beginner would not know as a hover/click tooltip — the user asked for "名词解释".
- **do** make every code block runnable or annotated, not just displayed. If the language has a runtime that fits in browser (JavaScript, Python via Pyodide, SQL, Lisp), embed a tiny REPL.

### Minimum deliverable shape

A teaching site worth shipping has all of:

1. persistent sidebar with chapter/section navigation
2. in-text glossary tooltip system (≥ 50 terms for a 21-day course)
3. progress persistence (localStorage — survive page reload)
4. at least 3 distinct interactive components that demonstrate the subject matter (calculator, REPL, state machine, canvas visualization, simulator)
5. full-text search over section titles and body
6. keyboard shortcuts (search, glossary, theme toggle)
7. dual theme (dark default + light) with one-click toggle
8. responsive collapse to mobile

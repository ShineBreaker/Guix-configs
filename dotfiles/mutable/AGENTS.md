...
| `agents/dsh`         | `~/.local/share/dsh/` + `~/.local/bin/dsh*`              | DeepSeek Harness 配置层 + 双 wrapper（`dsh` CLI / `dsh-web`）；本体经 pnpm 装在 `_cli/`，`$DSH_HOME` 由 conf.d 与 wrapper 双注入。细节规范见包内 `AGENTS.md` |

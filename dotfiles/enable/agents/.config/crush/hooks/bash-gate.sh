...
if echo "$CMD" | grep -qE '(^|\s)(vi|nano|pico|ed)\b'; then
	echo "禁止交互式编辑器，请使用 edit/write 工具" >&2
	exit 2
fi
if echo "$CMD" | grep -qE '(^|\s)(less|more|most|pg)\b'; then
	echo "禁止交互式 pager，请使用 view/grep 工具" >&2
	exit 2
fi
if echo "$CMD" | grep -qE '(^|\s)man\s'; then
	echo "禁止 man，请使用 --help 或在线文档" >&2
	exit 2
fi
...
if echo "$CMD" | grep -qE '(^|\s)git\s+rebase\s.*-i\b'; then
	echo "禁止 git rebase -i" >&2
	exit 2
fi
if echo "$CMD" | grep -qE '(^|\s)(python|python3)\s*$'; then
	echo "禁止裸 REPL，请使用 python -c '...' 或 python script.py" >&2
	exit 2
fi
if echo "$CMD" | grep -qE '(^|\s)node\s*$'; then
	echo "禁止裸 REPL，请使用 node -e '...' 或 node script.js" >&2
	exit 2
fi
if echo "$CMD" | grep -qE '(^|\s)ipython\b'; then
	echo "禁止 ipython，请使用 python -c '...'" >&2
	exit 2
fi

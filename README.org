...
+ *[[https://github.com/fishinthecalculator/sops-guix.git][sops-guix]]* - 密钥管理（sops + age 加密）
...
│   └── wm/.config/darkman/            # → ~/.config/darkman/
└── mutable/                            # Stow 管理（可独立调试）
    ├── emacs/.config/emacs/            # → ~/.config/emacs/
    ├── pi/.config/pi/                  # → ~/.config/pi/
    └── ssh/.ssh/                       # → ~/.ssh/
#+end_example

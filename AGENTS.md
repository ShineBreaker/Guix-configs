...
| 应用 dotfiles                   | `dotfiles/immutable/<app>/`（各子目录含 AGENTS.md）；`dotfiles/mutable/`（Stow 直链，见其 AGENTS.md）          |
...
4. **mutable 部署**（`dotfiles/mutable/`：emacs、lem、hermes、omp、secrets 等）：GNU Stow 直链仓库源码，改源即时生效，无需运行 `blue home`。
